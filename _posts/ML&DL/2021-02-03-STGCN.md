...
This article is about a review of the paper [[1]](https://arxiv.org/abs/1709.04875), and is the second part of a personal research project to apply Graph Neural Network (GNN) to the field of cascading outage prediction or detection.

I plan to use the model architecture used in this paper for cascading outage prediction.
...
- $\Lambda =diag([\lambda_0, ..., \lambda_{n-1}]) \in \mathbb{R}^{n\times n}$ is the matrix of eigenvalues of the normalized graph Laplacian $L$.
  ...
  (check details of Laplacian matrix in <a href="#appendix 6.1">appendix 6.1</a>)
...
(check details of graph Fourier transform in <a href="#appendix 6.2">appendix 6.2</a>)
...
(check details of Chebyshev polynomials approximation in <a href="#appendix 6.3">appendix 6.3</a>)
...
(check details of Localized convolutions in <a href="#appendix 6.4">appendix 6.4</a>)
...
<br>
<span style="color:blue">
A. It makes eigenvalues in the range [-1, 1] to prepare the recursive computation below.
</span>
<br>
<br>
...
<figure align="center">
  <img src="https://jhyun0919.github.io/assets/img/2021-02-03-STGCN/GLU.png" width="444" />
  <figcaption>Figure 4.  Gated Linear Unit (GLU). [12]</figcaption>
</figure>
...
**PeMSD7** was collected from Caltrans Performance Measurement System (PeMS) in real-time by over 39,000 sensor stations, deployed across the major metropolitan areas of California state highway system [Chen et al., 2001].
...
<a name="appendix 6.1"></a>

## 6.1. Laplacian & Symmetric normalized Laplacian [10]
...
<a name="appendix 6.3"></a>

## 6.3. Chebyshev Polynomial & Approximation [14, 15]
...
<a name="appendix 6.4"></a>

## 6.4. Localized Convolutional Filter
...
the simplest kernerl $\Theta(\cdot)$ is a non-parametic filter, i.e. the kernel parameters $\theta_k \in \mathbb{R}$  $(k=1, ..., n)$ are all free.
...
Two strategies to localize the filters in space. 😁

<br>

#### Restricting kernel to polynomial by Chebyshev approximation (Horizontal ver.)
...
keypoints
- $U$ is unitary matrix.
- $K$ as a radius of the conv filter.

<br>

#### Applying a stack of 1st-order approximated kernel (Vertical ver.)
