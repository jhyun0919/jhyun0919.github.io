...
Deep learning approaches have been widely and successfully applied to various traffic tasks nowadays.
...
In this work, the author defines the traffic network on a graph and focuses on structured traffic time series. The traffic network is represented as an undirected graph (or directed one) $\mathcal{G}$ with weights $w_{ij}$ as shown in Figure 1.
...
<figure align="center">
  ...
  <figcaption>Figure 1.  Graph-structured traffic data. Each v_t indicates a frame of current traffic status at time step t, which is recorded in a graph-structured data matrix. [1]</figcaption>
</figure>
...
A standard convolution for regular grids is clearly not applicable to general graphs. There are two basic approaches currently exploring how to generalize CNNs to structured data forms.

1. Expanding the spatial definition of a convolution [Niepert et al., 2016]

2. Manipulating in the spectral domain with graph Fourier transforms [Bruna et al., 2013]
