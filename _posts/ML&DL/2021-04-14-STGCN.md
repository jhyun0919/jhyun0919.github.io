...
We can localize the filter in space by restricting it to polynomial [8]. As a result, the filter is restricted as a polynomial filter. Moreover, by using Chebyshev approximation, we could limit the radius of convolutional filter.
